type SkeletonProps = {
	count: number;
};

const Skeleton = ({ count }: SkeletonProps) => {
	return Array.from({ length: count }, (_, index) => (
		<div key={index} className="border border-accent shadow rounded-md p-3 my-1 w-full mx-auto">
			<div className="animate-pulse flex space-x-4 items-center">
				<div className="rounded-xl bg-accent h-20 w-24 justify-center items-center"></div>
				<div className="flex-1 space-y-6 py-1">
					<div className="h-2 bg-accent rounded"></div>
					<div className="space-y-3">
						<div className="grid grid-cols-3 gap-4">
							<div className="h-2 bg-accent rounded col-span-2"></div>
							<div className="h-2 bg-accent rounded col-span-1"></div>
						</div>
						<div className="h-2 bg-accent rounded"></div>
					</div>
				</div>
			</div>
		</div>
	));
};

export default Skeleton;
